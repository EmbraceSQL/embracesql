...
The `context` parameter will have `headers` available, passed in via HTTP, or explicitly set with an embedded client library. The default implementation provided in the handler will:

* Look for `Authorization: Bearer <id_token>`
* Validate that token
* Set `context.token` on success
* Throw on failure
