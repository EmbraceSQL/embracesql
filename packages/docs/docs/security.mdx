...
You will need to get the provider's JSON Web Keys, which are JSON files with the keys stored encoded.

### JSON Web Keys

Configuration is file based, with keys stored as JSON Web Key files in the `${EMBRACE_SQL_ROOT}/.keys/<keyid>.json` folder.
You can have multiple `.json` files, and EmbraceSQL will try them for you, allowing multiple key providers if needed.

So `${EMBRACE_SQL_ROOT}/.keys/b16de1b2ab0c16ac0acf662ef01f7567e544252a.json` for example.

You can also use symmetric key, shared secret, encryption.
The difference is that both you and the issuer have the same secret, so don't lose it!
