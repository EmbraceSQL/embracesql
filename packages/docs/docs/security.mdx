...
However, there is an open standard that works great. OpenID Connect.
...
### Providers with OpenID Connect

EmbraceSQL will download and cache keys for you in the `${EMBRACE_SQL_ROOT}/.keys` folder.
This speeds authentication and verification by eliminating any need to go out over the network.

### Providers without OpenID Connect

Some providers don't yet support OpenID Connect, so you can supply your own copies of public keys.

Configuration is file based, with keys stored as `<issuer>/<keyid>.pem` files in the `${EMBRACE_SQL_ROOT}/.keys` folder.
You can have multiple `.pem` files, and EmbraceSQL will try them for you, allowing multiple key providers if needed.

You can also use symmetric key, shared secret, encryption. Just store the secret key in a `.pem`. The difference is that both you and the issuer have the same secret, so don't lose it!
...
```javascript title="${EMBRACE_SQL_ROOT}/default/things/all.sql.before.js"
...
When you call `context.allow()`, query execution is allowed.
...
```javascript title="${EMBRACE_SQL_ROOT}/before.js"
export const before = async (context) => {
  context.deny();
};
```
...
Let's further assume you have set up Google OpenID Connect to authenticate your users, and you are passing that id token in your `Authorization: Bearer ...` on requests to EmbraceSQL.
Google tokens have an `iss` issuer property that EmbraceSQL uses to automatically find the matching publc key, decodes, and verifies the token.
No need to store the public key in a `.pem`, it'll happen for you with a compliance OpenID Connect provider.

In the case that a token is valid, it is made available to your handlers.
In the case that a token is invalid, `403` is returned.
In the case that a token is not supplied, it simply is not available.
So checking authentication is checking for token existence.

```javascript title="${EMBRACE_SQL_ROOT}/all.before.js"
export const before = async (context) => {
...
```sql title="${EMBRACE_SQL_ROOT}/migrations/default/000_multitenant.sql"
-- SQLite style
...
```sql title="${EMBRACE_SQL_ROOT}/default/tenant.sql"
SELECT tenant_id WHERE domain = :domain
```

```sql title="${EMBRACE_SQL_ROOT}/default/my_things.sql"
...
Note that you don't need to create a database user for each possible user in PostgreSQL.
...
Migration support requires EmbraceSQL to create one table `_embrace_sql_migrations_` in each database.
