...
Keys are stored as JSON Web Key files named `${EMBRACE_SQL_ROOT}/.keys/<keyid>.json`.
...
```typescript title="${EMBRACE_SQL_ROOT}/before.ts" file=../../engine/tests/configs/security-secure-by-default/before.ts
``
...
```sql title="${EMBRACE_SQL_ROOT}/migrations/default/001_multitenant.sql"
